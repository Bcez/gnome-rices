...
var init = (function (gio2, clutter10, st1, meta10, gobject2, shell0, glib2, gda5, gdkpixbuf2, pango1, graphene1, soup3) {
    'use strict';
    ...
    const getMonitorConstraintForIndex = (index) => new imports.ui.layout.MonitorConstraint({
        index,
    });
    ...
    const monitorManager = meta10.MonitorManager.get();
    ...
    const debug$6 = logger('clipboard-manager');
    const MimeType = {
    ...
    let ClipboardManager = class ClipboardManager extends gobject2.Object {
        ...
        startTracking() {
            this.selectionChangedId = this.selection.connect('owner-changed', async (_, selectionType) => {
        ...
        async getContent() {
            return new Promise((resolve) => {
                const cbMimeTypes = this.clipboard.get_mimetypes(st1.ClipboardType.CLIPBOARD);
                if (this.haveMimeType(cbMimeTypes, MimeType.GNOME_FILE)) {
                    ...
                    this.clipboard.get_content(st1.ClipboardType.CLIPBOARD, currentMimeType, (_, bytes) => {
                    ...
                    this.clipboard.get_text(st1.ClipboardType.CLIPBOARD, (_, text) => {
                        if (text) {
                            resolve(new ClipboardContent({
    ...
    class ClipboardQueryBuilder {
        ...
        withId(id) {
            if (id !== null && id !== undefined) {
                this.conditions.push(this.builder.add_cond(gda5.SqlOperatorType.EQ, this.builder.add_field_id('id', 'clipboard'), this.add_expr_value(id), 0));
            }
            return this;
        }
        withItemTypes(itemTypes) {
            if (itemTypes !== null && itemTypes !== undefined) {
                const orConditions = itemTypes.map((itemType) => this.builder.add_cond(gda5.SqlOperatorType.EQ, this.builder.add_field_id('itemType', 'clipboard'), this.add_expr_value(itemType), 0));
        ...
        withContent(content) {
            if (content !== null && content !== undefined) {
                this.conditions.push(this.builder.add_cond(gda5.SqlOperatorType.EQ, this.builder.add_field_id('content', 'clipboard'), this.add_expr_value(content), 0));
            }
            return this;
        }
        withMatchValue(matchValue) {
            if (matchValue !== null && matchValue !== undefined) {
                this.conditions.push(this.builder.add_cond(gda5.SqlOperatorType.EQ, this.builder.add_field_id('matchValue', 'clipboard'), this.add_expr_value(matchValue), 0));
            }
            return this;
        }
        withContainingContent(content) {
            if (content !== null && content !== undefined) {
                this.conditions.push(this.builder.add_cond(gda5.SqlOperatorType.LIKE, this.builder.add_field_id('content', 'clipboard'), this.add_expr_value(`%${content}%`), 0));
            }
            return this;
        }
        withContainingSearchValue(searchValue) {
            if (searchValue !== null && searchValue !== undefined) {
                this.conditions.push(this.builder.add_cond(gda5.SqlOperatorType.LIKE, this.builder.add_field_id('searchValue', 'clipboard'), this.add_expr_value(`%${searchValue}%`), 0));
            }
            return this;
        }
        build() {
    ...
    `);
        ...
        update(dbItem) {
            ...
            builder.set_where(builder.add_cond(gda5.SqlOperatorType.EQ, builder.add_field_id('id', 'clipboard'), builder.add_expr_value(null, dbItem.id), 0));
        ...
        delete(id) {
            ...
            builder.set_table('clipboard');
            builder.set_where(builder.add_cond(gda5.SqlOperatorType.EQ, builder.add_field_id('id', 'clipboard'), builder.add_expr_value(null, id), 0));
            this.connection.statement_execute_non_select(builder.get_statement(), null);
        }
    ...
    let PanoItem = class PanoItem extends st1.BoxLayout {
        constructor(dbItem) {
            ...
            this.indicatorBox = new st1.BoxLayout({
                visible: true,
    ...
    const getColor = (classNames) => {
        ...
        if (!item) {
            debug$4(`class names not found: ${classNames}`);
            return '#fff';
        }
        return item.fgcolor;
    };
    ...
    const debug$3 = logger('link-parser');
    const getDocument = async (url) => {
        ...
        try {
            ...
            if (response == null) {
                debug$3(`no response from ${url}`);
                return defaultResult;
            }
            const bytes = response.get_data();
            if (bytes == null) {
                debug$3(`no data from ${url}`);
                return defaultResult;
            }
            ...
            debug$3(`failed to parse link ${url}. err: ${err}`);
        }
        return defaultResult;
    };
    const getImage = async (imageUrl) => {
        if (imageUrl && imageUrl.startsWith('http')) {
            try {
                ...
                const response = await session.send_and_read_async(message, glib2.PRIORITY_DEFAULT, null);
                if (!response) {
                    debug$3('no response while fetching the image');
                    return [null, null];
                }
                const data = response.get_data();
                if (!data || data.length == 0) {
                    debug$3('empty response while fetching the image');
                    return [null, null];
                }
                ...
                debug$3(`failed to load image: ${imageUrl}. err: ${err}`);
            }
        }
        return [null, null];
    };
    ...
    const debug$2 = logger('pano-item-factory');
    const findOrCreateDbItem = async (clip) => {
        ...
        switch (type) {
            ...
            case ContentType.TEXT:
                if (value.trim().toLowerCase().startsWith('http') && isUrl_1(value)) {
    ...
    class KeyManager {
        ...
        stopListening() {
            Object.keys(this.grabbers).forEach((grabberAction) => {
                ...
                shell0.Global.get().display.ungrab_accelerator(grabber.action);
                wm.allowKeybinding(grabber.name, shell0.ActionMode.NONE);
            });
            if (this.acceleratorActivatedId !== null) {
                shell0.Global.get().display.disconnect(this.acceleratorActivatedId);
                this.acceleratorActivatedId = null;
            }
        }
        listenFor(accelerator, callback) {
            this.acceleratorActivatedId = shell0.Global.get().display.connect('accelerator-activated', (_, action) => {
                this.onAccelerator(action);
            });
            debug$1(`Trying to listen for hot key [accelerator=${accelerator}]`);
            const action = shell0.Global.get().display.grab_accelerator(accelerator, meta10.KeyBindingFlags.NONE);
            if (action == meta10.KeyBindingAction.NONE) {
                debug$1(`Unable to grab accelerator [binding=${accelerator}]`);
            }
            else {
                debug$1(`Grabbed accelerator [action=${action}]`);
                const name = meta10.external_binding_name_for_action(action);
                debug$1(`Received binding name for action [name=${name}, action=${action}]`);
                wm.allowKeybinding(name, shell0.ActionMode.ALL);
...
})(imports.gi.Gio, imports.gi.Clutter, imports.gi.St, imports.gi.Meta, imports.gi.GObject, imports.gi.Shell, imports.gi.GLib, imports.gi.Gda, imports.gi.GdkPixbuf, imports.gi.Pango, imports.gi.Graphene, imports.gi.Soup);
